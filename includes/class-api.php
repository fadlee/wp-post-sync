...
class WP_Post_Sync_API {
    ...
    public function check_api_permission($request) {
        ...
        $body_api_key = $data['auth'] ?? '';

        if (!$auth_header || !$body_api_key) {
            return false;
        }

        $header_api_key = str_replace('Bearer ', '', $auth_header);
        $saved_api_key = get_option('wp_post_sync_api_key');

        return $header_api_key === $saved_api_key || $body_api_key === $saved_api_key;
    }
